...
'
'  The Manager runs as a WinForms app. Microsoft.Extensions.Logging's
'  AddConsole() writes to a console stream that simply isn't attached
'  in GUI mode, so Info/Warning/Error lines have been silently dropped
'  since day one.
'
'  This provider writes one UTF-8 text file per day under
'  {app}/logs/manager-YYYY-MM-DD.log. Writes are serialised through
'  a single lock — throughput is fine for diagnostic logging, and
'  the simplicity means we don't accumulate unflushed buffers on
'  unclean shutdown.
'
'  If you outgrow this (need structured logs, size-based rolling,
'  multiple sinks), swap in Serilog — two NuGet packages and five
'  lines of setup code. For now this keeps the project free of
'  third-party logging dependencies.
' ============================================================
...
Namespace GSM.Manager
    ...
    Public NotInheritable Class FileLoggerProvider
        ...
        Public Sub New(directory As String,
                       ...
                       Optional filenamePrefix As String = "manager-")
            _directory = directory
            _minLevel = minLevel
            _filenamePrefix = If(filenamePrefix, "manager-")
            Try
        ...
        ''' ("manager-2026-04-21.log.zip", say) and we shouldn't
        ''' delete that. Files where the date parse fails are
        ''' similarly skipped.
        '''
