...
- **myrealm realm onboarding & import (7-6).** Turns the 7-5b
  ...
  **History consistency:** `SourceLabelContext` gained
  ...
  no ContractsVersion bump. **Deferred:** the persisted session→realm
  access map + multi-session routing (decision 2) — no topology to
  exercise it yet. Realm ADMINISTRATION (writes) is Phase 10.
